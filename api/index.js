...
// ---- Viáticos no realizado: custo de viagem, sem o dinheiro em trânsito ----
//
// São duas categorias com papéis opostos em Contas a Pagar:
//
//   "Repasse de Viáticos" -> carga da carteira Flash. NÃO é custo: o dinheiro
//        troca de lugar (conta -> cartão) e continua sendo da empresa. Fica
//        só no Fluxo de Caixa, onde a pergunta é quando o dinheiro saiu.
//   "Viáticos"            -> reembolso ao colaborador que pagou do bolso
//        (cartão recusado, pedágio, adiantamento). Isso É custo, e muitas
//        dessas viagens não têm comprovante lançado na seção Viáticos.
//
// Então o realizado soma as DUAS fontes:
//
//   realizado(Viáticos) = despesas comprovadas na seção Viáticos
//                       + reembolsos pagos na categoria "Viáticos"
//
// A data é a da DESPESA (e a do pagamento, no reembolso), não a do repasse: um
// almoço em 05/08 é custo de agosto, tenha o cartão sido carregado em julho.
//
// Consequência que precisa estar visível na tela: viagem paga por cartão e sem
// comprovante lançado entra como R$ 0. O número depende da comprovação em dia.
const CAT_VIATICOS = 'Viáticos';
const CAT_REPASSE_VIATICOS = 'Repasse de Viáticos';
...
// Somar por (mês, categoria) ANTES de responder. Viáticos passa a chegar por
// duas fontes, e há consumidor que faz `.find(...)` por categoria em vez de
// acumular — com duas linhas, esse pegaria só a primeira e perderia a outra.
function somarPorMesCategoria(linhas) {
  const m = new Map();
  linhas.forEach(r => {
    const k = r.month + '|' + r.category;
    m.set(k, (m.get(k) || 0) + n(r.total));
  });
  return [...m].map(([k, total]) => {
    const i = k.indexOf('|');
    return { month: Number(k.slice(0, i)), category: k.slice(i + 1), total };
  });
}
...
app.get('/api/reports/actuals/:year', requireAuth, requireViewAny(['orcadoreal','relatorios']), h(async (req, res) => {
  ...
  // O repasse à carteira Flash sai do realizado. E sai da resposta inteira, não
  // só da soma: sem orçamento próprio, ele apareceria na tela como uma
  // categoria "Sem orçamento" de seis dígitos.
  const despesas = await query(`
    ...
    GROUP BY month, category`, [y, CAT_REPASSE_VIATICOS]);
  ...
  // Contexto para a tela explicar o número em vez de só mostrá-lo: as duas
  // fontes do custo, e o repasse que ficou de fora.
  const reembolsado = n((await query(
  ...
  const repassado = n((await query(
    `SELECT COALESCE(SUM(amount),0) AS v FROM erp_payables
      WHERE status='pago' AND category=$1 AND EXTRACT(YEAR FROM payment_date)=$2`, [CAT_REPASSE_VIATICOS, y]))[0].v);
  ...
  res.json({
    despesas: somarPorMesCategoria(despesas
      .concat(viaticos.map(r => ({ month: r.month, category: CAT_VIATICOS, total: r.total })))),
    ...
    viaticos: {
      ...
      reembolsado,
      repassado,
      categoria_repasse: CAT_REPASSE_VIATICOS,
...
app.get('/api/reports/dashboard', requireAuth, requireViewAny(['dashboard']), h(async (req, res) => {
  ...
  const realCatRows = somarPorMesCategoria(
    (await query(`SELECT category, SUM(amount) AS total FROM erp_payables
       ...
       GROUP BY category`, [mesAtual, CAT_REPASSE_VIATICOS]))
...
// ---- Organograma ----
//
// Não é tabela nova: `gestor_id` já existia em erp_rh_vinculos desde que o
// vínculo foi desenhado, mas nenhuma tela jamais deixou preenchê-lo — a
// aresta que faz um organograma existir estava pronta e nunca foi usada. A
// árvore em si é montada no CLIENTE, a partir da lista plana: é o mesmo
// desenho que o resto do RH já usa (a tela recebe linhas e monta a estrutura),
// e evita mandar uma árvore serializada que ninguém mais consome.
app.get('/api/rh/organograma', requireAuth, requireViewAny(['rh']), h(async (req, res) => {
  const pessoas = await query(`
    SELECT c.id, c.name, v.cargo, v.nivel, v.departamento, v.gestor_id
      FROM erp_colaboradores c
      JOIN erp_rh_vinculos v ON v.colaborador_id = c.id AND v.desligamento IS NULL
     WHERE c.ativo = true AND c.arquivado_em IS NULL
     ORDER BY c.name`);
  // gestor_id pode apontar para alguém que SAIU da lista (foi desligado,
  // arquivado) sem que ninguém tenha atualizado quem essa pessoa passou a
  // reportar. O organograma não pode inventar uma caixa vazia para o
  // ex-gestor — melhor tratar como "sem gestor" e AVISAR, do mesmo jeito que
...
// Ninguém reporta para si mesmo, e a cadeia de gestores não pode fechar um
// ciclo (A reporta pra B que reporta pra A) — o organograma não sabe desenhar
// isso, só travaria numa recursão sem fim. Anda a cadeia ACIMA do gestor
// escolhido; se ela voltar para quem está sendo editado, é ciclo.
async function rhValidarGestor(colaboradorId, gestorIdBruto) {
  if (gestorIdBruto === undefined || gestorIdBruto === null || gestorIdBruto === '') return null;
  const gestorId = Number(gestorIdBruto);
  if (!Number.isFinite(gestorId)) return null;   // valor inválido: rhMontarSet descarta, nada a validar aqui
  if (gestorId === colaboradorId) return 'Ninguém pode reportar para si mesmo.';
  let atualId = gestorId, guarda = 0;
  while (atualId && guarda++ < 100) {             // 100: nenhuma empresa real tem cadeia hierárquica tão funda
    if (atualId === colaboradorId) {
      return 'Essa escolha criaria um ciclo — quem seria o gestor já está, direta ou indiretamente, subordinado a esta pessoa.';
    }
    const acima = await query(
      'SELECT gestor_id FROM erp_rh_vinculos WHERE colaborador_id=$1 AND desligamento IS NULL LIMIT 1', [atualId]);
    atualId = acima.length ? acima[0].gestor_id : null;
  }
  return null;
}
...
app.post('/api/rh/colaboradores/:id/vinculos', requireAuth, requireEdit('rh'), h(async (req, res) => {
  ...
  const erroGestor = await rhValidarGestor(id, req.body.gestor_id);
  if (erroGestor) return res.status(400).json({ error: erroGestor });
...
app.put('/api/rh/vinculos/:id', requireAuth, requireEdit('rh'), h(async (req, res) => {
  ...
  const erroGestor = await rhValidarGestor(atual[0].colaborador_id, req.body.gestor_id);
  if (erroGestor) return res.status(400).json({ error: erroGestor });
