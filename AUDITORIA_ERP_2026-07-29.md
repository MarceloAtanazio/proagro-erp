...
>
> **Fase 1 — Lançamento do autosserviço (30/07/2026):**
> - ✅ **A4 fechado** — a Fase 0 apenas conteve o acesso (só super-admin); agora o servidor **recalcula** `categoria_local` e `previsao_por_categoria` a partir de destinos/período/itens de transporte BRUTOS (`viaRecalcularPrevisao`, com a mesma regra de cidade-sede do A1), e ignora por completo os totais prontos que o navegador envia. Verificado com payload malicioso real (`previsao_por_categoria: {hospedagem:99999,...}` + `categoria_local:'interior'` forjado): o servidor gravou o valor recalculado correto (`{"taxi_uber":555,"alimentacao":420}`, categoria `sp_df_rj_intl`), ignorando os valores injetados; dado de teste removido do banco após a checagem.
> - A seção deixou de ser rota solta (`#via-solicitar`): passou a viver **dentro de Viáticos**, com botão "Solicitar viagem" visível a qualquer usuário com acesso à página e vinculado a um colaborador ativo. Bookmarks antigos redirecionam e abrem o assistente automaticamente — verificado no navegador.
>
